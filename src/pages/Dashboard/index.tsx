...
const Dashboard: React.FC = () => {
    ...
    const months = useMemo(() => {
        return Array.from(
            new Set(
                [...gains, ...expenses].map(item => moment(item.date, "YYYY-MM-DD").format('M'))
            )
    ...
    const years = useMemo(() => {
        return Array.from(
            new Set([...gains, ...expenses].map(item => moment(item.date, "YYYY-MM-DD").year())
            )
        ...
        return total < 0 ?
            ...
            } :
            {
                ...
                message: {
                    ...
                    footerText: "Continue assim. Considere investir seu saldo quando for possível.",
                    icon: happyImg
                }
            }
    ...
    const historyData = useMemo(() => {
        return moment.monthsShort().map(month => {
            ...
            gains.forEach(gain => {
                const date = moment(gain.date, "YYYY-MM-DD");
                if (date.format("MMM") === month && String(date.year()) === selectedYear) {
        ...
        }).filter(item => selectedYear === String(moment().year())
